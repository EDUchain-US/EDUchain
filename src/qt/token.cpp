...
bool ToHash160(const std::string& strEdcAddress, std::string& strHash160)
{   
    CTxDestination EDCAddress = DecodeDestination(strEdcAddress);
    if(IsValidDestination(EDCAddress)){     
        CKeyID *keyid = boost::get<CKeyID>(&EDCAddress);
...
bool ToEdcAddress(const std::string& strHash160, std::string& strEdcAddress)
{
    uint160 key(ParseHex(strHash160.c_str()));
    CKeyID keyid(key);
    
    if(IsValidDestination(CTxDestination(keyid))){
        strEdcAddress = EncodeDestination(CTxDestination(keyid));
        return true;
    }
    return false;
}
...
bool Token::execEvents(int64_t fromBlock, int64_t toBlock, int func, std::vector<TokenEvent> &tokenEvents)
{
    ...
    for(int i = 0; i < list.size(); i++)
    {
        ...
        for(int i = 0; i < listLog.size(); i++)
        {
            ...
            ToEdcAddress(tokenEvent.sender, tokenEvent.sender);
            tokenEvent.receiver = topicsList[2].toString().toStdString().substr(24);
            ToEdcAddress(tokenEvent.receiver, tokenEvent.receiver);
