...
void SendTokenPage::on_numBlocksChanged()
{
    if(m_clientModel)
    {
        ...
        ui->labelGasPrice->setToolTip(tr("Gas price: EDC/gas unit. Default = %1, Min = %2.").arg(QString::fromStdString(FormatMoney(DEFAULT_GAS_PRICE))).arg(QString::fromStdString(FormatMoney(minGasPrice))));
...
WalletModel::UnlockContext ctx(m_model->requestUnlock());
    ...
    {
        QString message = tr("To send %1 you need EDC in address <br /> %2.")
