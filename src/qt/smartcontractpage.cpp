...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::EDC)
    {
    }
                                              ...
                                              filter(0)
{
    ...
    if (fLiteMode) {
        ...
        if (fMasterNode) {
            ...
            if (!fEnableEdcsend) {
                ui->toggleObfuscation->setText(tr("Start Edcsend"));
            } else {
                ui->toggleObfuscation->setText(tr("Stop Edcsend"));
            }
...
void SmartContractPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("EDC")
    updateDisplayUnit();
}
...
void SmartContractPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeEdcAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeEdcAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeEdcAmount = strAnonymizeEdcAmount.remove(strAnonymizeEdcAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeEdcAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeEdcAmount * COIN) nMaxToAnonymize = nAnonymizeEdcAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeEdcAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeEdcAmount));
        strAnonymizeEdcAmount = strAnonymizeEdcAmount.remove(strAnonymizeEdcAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeEdcAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
