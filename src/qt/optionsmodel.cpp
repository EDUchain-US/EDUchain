...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::EDC);
    ...
    if (!settings.contains("nAnonymizeEdcAmount"))
        settings.setValue("nAnonymizeEdcAmount", 1000);

    nDarksendRounds = settings.value("nDarksendRounds").toLongLong();
    nAnonymizeEdcAmount = settings.value("nAnonymizeEdcAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeEdcAmount"))
        SoftSetArg("-anonymizeEDCamount", settings.value("nAnonymizeEdcAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in EDC.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeEdcAmount:
            return QVariant(nAnonymizeEdcAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeEdcAmount:
            nAnonymizeEdcAmount = value.toInt();
            settings.setValue("nAnonymizeEdcAmount", nAnonymizeEdcAmount);
            emit anonymizeEdcAmountChanged(nAnonymizeEdcAmount);
            break;
        case CoinControlFeatures:
