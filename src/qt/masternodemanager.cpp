...
static void NotifyEdcNodeUpdated(MasternodeManager *page, CEdcNodeConfig nodeConfig)
{
    ...
    QMetaObject::invokeMethod(page, "updateEdcNode", Qt::QueuedConnection,
                              Q_ARG(QString, alias),
                              Q_ARG(QString, addr),
...
void MasternodeManager::subscribeToCoreSignals()
{
    ...
    uiInterface.NotifyEdcNodeChanged.connect(boost::bind(&NotifyEdcNodeUpdated, this, _1));
}

void MasternodeManager::unsubscribeFromCoreSignals()
{
    // Disconnect signals from core
    uiInterface.NotifyEdcNodeChanged.disconnect(boost::bind(&NotifyEdcNodeUpdated, this, _1));
}
...
void MasternodeManager::updateEdcNode(QString alias, QString addr, QString privkey, QString collateral)
{
    LOCK(cs_adrenaline);
    bool bFound = false;
    int nodeRow = 0;
...
void MasternodeManager::updateNodeList()
{
    ...
    {
        LOCK(cs_adrenaline);
        BOOST_FOREACH(PAIRTYPE(std::string, CEdcNodeConfig) adrenaline, pwalletMain->mapMyEdcNodes)
        {
            updateEdcNode(QString::fromStdString(adrenaline.second.sAlias), QString::fromStdString(adrenaline.second.sAddress), QString::fromStdString(adrenaline.second.sMasternodePrivKey), QString::fromStdString(adrenaline.second.sCollateralAddress));
        }
    }
}
...
{
    AddEditEdcNode* aenode = new AddEditEdcNode();
    aenode->exec();
}
...
void MasternodeManager::on_getConfigButton_clicked()
{
    ...
    int r = index.row();
    std::string sAddress = ui->tableWidget_2->item(r, 1)->text().toStdString();
    CEdcNodeConfig c = pwalletMain->mapMyEdcNodes[sAddress];
    std::string sPrivKey = c.sMasternodePrivKey;
    EdcNodeConfigDialog* d = new EdcNodeConfigDialog(this, QString::fromStdString(sAddress), QString::fromStdString(sPrivKey));
    d->exec();
}
...
void MasternodeManager::on_removeButton_clicked()
{
    ...
    if(confirm == QMessageBox::Yes)
    {
        ...
        pwalletMain->mapMyEdcNodes.erase(sAddress);
        walletdb.EraseEdcNodeConfig(c.sAddress);
...
void MasternodeManager::on_startAllButton_clicked()
{
    ...
    BOOST_FOREACH(PAIRTYPE(std::string, CEdcNodeConfig) adrenaline, pwalletMain->mapMyEdcNodes)
    {
        CEdcNodeConfig c = adrenaline.second;
	std::string errorMessage;
        bool result = activeMasternode.RegisterByPubKey(c.sAddress, c.sMasternodePrivKey, c.sCollateralAddress, errorMessage);
	if(result)
	{
