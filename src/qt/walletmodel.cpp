...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered EDC address / amount:
...

    {
        ...
        if (recipients[0].useInstanTX && total > GetSporkValue(SPORK_2_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("InstanTX doesn't support sending values that high yet. Transactions are currently limited to %1 EDC.").arg(GetSporkValue(SPORK_2_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal EDC:URI (EDC:XyZ...?message=example)
            {
...
bool WalletModel::isUnspentAddress(const std::string &EDCAddress) const
{
    LOCK2(cs_main, wallet->cs_wallet);
    ...
    BOOST_FOREACH(const COutput& out, vecOutputs)
    {
        ...
        if(fValidAddress && EncodeDestination(address) == EDCAddress && out.tx->vout[out.i].nValue)
        {
            return true;
        }
    }
