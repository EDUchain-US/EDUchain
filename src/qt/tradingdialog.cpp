        ...
        model(0)
{
    ...
    ui->EDCAvailableLabel->setTextFormat(Qt::RichText);
    ...
    ui->CryptopiaEDCLabel->setTextFormat(Qt::RichText);
...
//    connect(ui->TradingTabWidget, SIGNAL(tabBarClicked(int)), this, SLOT(on_TradingTabWidget_tabBarClicked(int)));
    ...
    CreateOrderBookTables(*ui->BidsTable,QStringList() << "SUM(BTC)" << "TOTAL(BTC)" << "EDC(SIZE)" << "BID(BTC)");
    CreateOrderBookTables(*ui->AsksTable,QStringList() << "ASK(BTC)" << "EDC(SIZE)" << "TOTAL(BTC)" << "SUM(BTC)");
    ...
    ui->MarketHistoryTable->setHorizontalHeaderLabels(QStringList()<<"DATE"<<"BUY/SELL"<<"BID/ASK"<<"TOTAL UNITS(EDC)"<<"TOTAL COST(BTC)");
...
void tradingDialog::UpdaterFunction(){
    //EDCst get the main exchange info in order to populate qLabels in maindialog. then get data
...
QString tradingDialog::GetMarketSummary(){
    ...
    QString Response = sendRequest("https://www.cryptopia.co.nz/api/GetMarket/EDC_BTC");
    return Response;
}

QString tradingDialog::GetOrderBook(){

    QString  Response = sendRequest("https://www.cryptopia.co.nz/api/GetMarketOrders/EDC_BTC");
    return Response;
}
...
void tradingDialog::GetMarketHistory(){
    ...
    sendRequest1(QString("https://www.cryptopia.co.nz/api/GetMarketHistory/EDC_BTC"), f);
}
...
QString tradingDialog::BuyEDC(QString OrderType, double Quantity, double Rate){

    QString str = "";
    QString URL = "https://www.cryptopia.co.nz/api/SubmitTrade";

    QJsonObject stats_obj;
    stats_obj["Market"] = "EDC/BTC";
    stats_obj["Type"] = "Buy";
...
QString tradingDialog::SellEDC(QString OrderType, double Quantity, double Rate){
...
QString tradingDialog::GetDepositAddress(){
    ...
    QString Response = sendRequest(URL, "POST", QString("{\"Currency\":\"EDC\"}"));
    return Response;
}
...
int tradingDialog::SetExchangeInfoTextLabels(){
    ...
    ui->volumet->setText("<b>EDC Volume:</b> <span style='font-weight:bold; font-size:12px; color:blue;'>" + str.number(obj["Volume"].toDouble(),'i',8) + "</span> EDC");
...
void tradingDialog::AskInfoInsertSlot(int row, int col){
    ...
    QString UnitsEDCString = ui->AsksTable->model()->data(ui->AsksTable->model()->index(row,1)).toString();
    ui->SellBidPriceEdit->setText(SellBidPriceString);
    ui->UnitsInputEDC->setText(UnitsEDCString);
...
void tradingDialog::ParseAndPopulateOrderBookTables(QString OrderBook){
    ...
    ui->EDCSupply->setText("<b>Supply:</b> <span style='font-weight:bold; font-size:12px; color:blue'>" + str.number(EDCSupply,'i',8) + "</span><b> EDC</b>");
    ...
    ui->EDCDemand->setText("<b>Demand:</b> <span style='font-weight:bold; font-size:12px; color:blue'>" + str.number(EDCDemand,'i',8) + "</span><b> EDC</b>");
...
void tradingDialog::ParseAndPopulatePriceChart(QString Response){
    ...
    derPlot->yAxis->setLabel("EDC Price in BTC");
...
void tradingDialog::ActionsOnSwitch(int index = -1){
    ...
    switch (index){
        case 0:    //buy tab is active
            ...
            f = std::bind(&tradingDialog::showBalanceOfEDCOnTradingTab, this);
            sendRequest1(QString("https://www.cryptopia.co.nz/api/GetBalance"), f, "POST", QString("{\"Currency\":\"") + "EDC" + QString("\"}"));
            ...
            sendRequest1("https://www.cryptopia.co.nz/api/GetMarketOrders/EDC_BTC", f);
        ...
        case 1: //Cross send tab active
            ...
            f = std::bind(&tradingDialog::showBalanceOfEDCOnSendTab, this);
            sendRequest1(QString("https://www.cryptopia.co.nz/api/GetBalance"), f, "POST", QString("{\"Currency\":\"") + "EDC" + QString("\"}"));
        ...
        case 5://show balance tab
            ...
            f = std::bind(&tradingDialog::showBalanceOfEDCOnBalanceTab, this);
            sendRequest1(QString("https://www.cryptopia.co.nz/api/GetBalance"), f, "POST", QString("{\"Currency\":\"") + "EDC" + QString("\"}"));
...
void tradingDialog::showBalanceOfEDCOnTradingTab() {
    QNetworkReply *reply = qobject_cast<QNetworkReply *>(sender());
    QString Response = "";
    if (reply->error() == QNetworkReply::NoError) {
        //success
    ...
    reply->deleteLater();
    if(Response.size() > 0 && Response != "Error"){
        DisplayBalance(*ui->EDCAvailableLabel, Response);
    }
}
...
void tradingDialog::showBalanceOfEDCOnSendTab() {
    QNetworkReply *reply = qobject_cast<QNetworkReply *>(sender());
    QString Response = "";
    if (reply->error() == QNetworkReply::NoError) {
        //success
    ...
    reply->deleteLater();
    if(Response.size() > 0 && Response != "Error"){
        DisplayBalance(*ui->CryptopiaEDCLabel, Response);
    }
}
...
void tradingDialog::showBalanceOfEDCOnBalanceTab() {
    QNetworkReply *reply = qobject_cast<QNetworkReply *>(sender());
    QString Response = "";
    if (reply->error() == QNetworkReply::NoError) {
        //success
    ...
    reply->deleteLater();
    if(Response.size() > 0 && Response != "Error"){
        DisplayBalance(*ui->EDCBalanceLabel,*ui->EDCAvailableLabel_2,*ui->EDCPendingLabel, QString::fromUtf8("EDC"),Response);
    }
}
...
void tradingDialog::CalculateSellCostLabel(){
    ...
    double Quantity = ui->UnitsInputEDC->text().toDouble();
    double cost = ((price * Quantity) - ((price * Quantity / 100) * 0.25));
...
void tradingDialog::CalculateCSReceiveLabel(){
    ...
    QString balance = GetBalance("EDC");
    ...
    double AvailableEDC = BalanceObject["Available"].toDouble();
...
{
    //calculate amount of BTC that can be gained from selling EDC available balance
    QString responseA = GetBalance("EDC");
    QString str;
    QJsonObject ResultObject =  GetResultObjectFromJSONArray(responseA);

    double AvailableEDC = ResultObject["Available"].toDouble();

    ui->UnitsInputEDC->setText(str.number(AvailableEDC,'i',8));
}
...
{
    double Quantity = ui->CryptopiaEDCLabel->text().toDouble();
    double Received = 0;
    double Qty = 0;
    double Price = 0;
    QString buyorders = GetOrderBook();
...
void tradingDialog::on_Withdraw_Max_Amount_clicked()
{
    ...
    ui->WithdrawUnitsInput->setText(str.number(AvailableEDC,'i',8));
}
...
void tradingDialog::on_BuyEDC_clicked()
{
    double Rate;
    double Quantity;
    ...
    if (reply == QMessageBox::Yes) {
        ...
        QString Response =  BuyEDC(Order,Quantity,Rate);
...
void tradingDialog::on_SellEDCBTN_clicked()
{
    double Rate;
    double Quantity;

    Rate     = ui->SellBidPriceEdit->text().toDouble();
    Quantity = ui->UnitsInputEDC->text().toDouble();
    ...
    if (reply == QMessageBox::Yes) {
        ...
        QString Response =  SellEDC(Order,Quantity,Rate);
        QJsonDocument jsonResponse = QJsonDocument::fromJson(Response.toUtf8());          //get json from str.
        QJsonObject  ResponseObject = jsonResponse.object();                              //get json obj
...
void tradingDialog::on_CSUnitsBtn_clicked()
{
    ...
    foreach (const QJsonValue & value, BuyArray)
    {
        ...
        if ( ((Quantity / x) - y) > 0 )
        {
            ...
            QString SellResponse = SellEDC(Order,y,x);
            ...
            QString SellResponse = SellEDC(Order,(Quantity / x),x);
            QJsonDocument SelljsonResponse = QJsonDocument::fromJson(SellResponse.toUtf8());          //get json from str.
            QJsonObject SellResponseObject = SelljsonResponse.object();                              //get json obj
            ...
            if (SellResponseObject["Success"].toBool() == false){
                ...
                if (ResponseObject["Success"].toBool() == false){
                    ...
                    if (ResponseObject["Success"].toBool() == false){
                        ...
                        QMessageBox::information(this,"Success","<center>Cross-Send Successful</center>\n Sold "+Astr.number(Qty,'i',4)+" EDC for "+Qstr.number((ui->CSUnitsInput->text().toDouble()-0.0002),'i',8)+" BTC");
                    }
                } else if (ResponseObject["Success"].toBool() == true){
                    QMessageBox::information(this,"Success","<center>Cross-Send Successful</center>\n Sold "+Astr.number(Qty,'i',4)+" EDC for "+Qstr.number((ui->CSUnitsInput->text().toDouble()-0.0002),'i',8)+" BTC");
                }
...
void tradingDialog::on_UnitsInputEDC_textChanged(const QString &arg1)
{
    CalculateSellCostLabel(); //update cost
}
