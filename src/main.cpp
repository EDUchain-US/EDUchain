...
#if defined(NDEBUG)
#error "EDC cannot be compiled without assertions."
#endif
...
std::unique_ptr<EdcState> globalState;
...
unsigned int dgpMaxBlockSize = 2000000; // EDC
...
const string strMessageMagic = "Edc Signed Message:\n";
...
#                   endif
        ...
        if(chainActive.Height() >= chainparams.FirstSCBlock() && tx.HasCreateOrCall()) {
            ...
            EdcDGP EDCDGP(globalState.get(), fGettingValuesDGP);
            uint64_t minGasPrice = EDCDGP.getMinGasPrice(chainActive.Tip()->nHeight + 1);
            uint64_t blockGasLimit = EDCDGP.getBlockGasLimit(chainActive.Tip()->nHeight + 1);
            size_t count = 0;
            for(const CTxOut& o : tx.vout)
                count += o.scriptPubKey.HasOpCreate() || o.scriptPubKey.HasOpCall() ? 1 : 0;
            EdcTxConverter converter(tx, NULL);
            ExtractEdcTX resultConverter;
            if(!converter.extractionEdcTransactions(resultConverter)){
            ...
            std::vector<EdcTransaction> EDCTransactions = resultConverter.first;
            std::vector<EthTransactionParams> EDCETP = resultConverter.second;

            dev::u256 sumGas = dev::u256(0);
            dev::u256 gasAllTxs = dev::u256(0);
            for(EdcTransaction EDCTransaction : EDCTransactions){
                sumGas += EDCTransaction.gas() * EDCTransaction.gasPrice();
                ...
                if(txMinGasPrice != 0) {
                    txMinGasPrice = std::min(txMinGasPrice, EDCTransaction.gasPrice());
                } else {
                    txMinGasPrice = EDCTransaction.gasPrice();
                }
                VersionVM v = EDCTransaction.getVersion();
                if(v.format!=0)
                ...
                if(EDCTransaction.gas() < GetArg("-minmempoolgaslimit", MEMPOOL_MIN_GAS_LIMIT))
                ...
                if(EDCTransaction.gas() < MINIMUM_GAS_LIMIT && v.rootVM != 0)
                ...
                if(EDCTransaction.gas() > UINT32_MAX)
                ...
                if(v.rootVM!=0 && (uint64_t)EDCTransaction.gasPrice() < minGasPrice)
            ...
            if(!CheckMinGasPrice(EDCETP, minGasPrice))
                return state.DoS(100, false, REJECT_INVALID, "bad-txns-small-gasprice");

            if(count > EDCTransactions.size())
...
CAmount GetProofOfWorkReward(int64_t nFees, int nHeight)
{
    if (Params().NetworkID() == CBaseChainParams::TESTNET) {
        if (nHeight < 200) return 250000 * COIN; //testnet
    }
    ...
    if (nHeight < 1) {
        ...
        nSubsidy = 2512000 * COIN; //premine / instamine
    } else if (nHeight < 999) {
        nSubsidy = 1 * COIN;
    } else if (nHeight < 2999) {
        nSubsidy = 1.5 * COIN;
    } else if (nHeight < 4999) {
        nSubsidy = 2 * COIN;
    } else if (nHeight < 6999) {
        nSubsidy = 4 * COIN;
    } else if (nHeight < 8999) {
        nSubsidy = 8 * COIN;
    } else if (nHeight < 19999) {
        nSubsidy = 16 * COIN;
    } else if (nHeight < 39999) {
        nSubsidy = 32 * COIN;
    } else if (nHeight < 59999) {
        nSubsidy = 64 * COIN;
    } else if (nHeight < 79999) {
        nSubsidy = 128 * COIN;
    } else if (nHeight < 99999) {
        nSubsidy = 256 * COIN;
    } else if (nHeight = 150000) { //BONUS SUPER BLOCK
        nSubsidy = 1024 * COIN;
    } else if (nHeight = 200000) { //BONUS SUPER BLOCK
        nSubsidy = 2048 * COIN;
    } else if (nHeight = 300000) { //BONUS SUPER BLOCK
        nSubsidy = 2048 * COIN;
    } else if (nHeight = 350000) { //BONUS SUPER BLOCK
        nSubsidy = 4096 * COIN;
    } else if (nHeight = 400000) { //BONUS SUPER BLOCK
        nSubsidy = 4096 * COIN;
    } else if (nHeight = 450000) { //BONUS SUPER BLOCK
        nSubsidy = 8192 * COIN;
    } else if (nHeight = 500000) { //BONUS SUPER BLOCK
        nSubsidy = 8192 * COIN;
    } else {
        nSubsidy = 1 * COIN;
    }
...
//#if 0
      if (pindex->nHeight > Params().FirstSCBlock()) {
        globalState->setRoot(uintToh256(pindex->pprev->hashStateRoot)); // EDC
        globalState->setRootUTXO(uintToh256(pindex->pprev->hashUTXORoot)); // EDC
...
#if 0
    EdcDGP EDCDGP(globalState.get(), fGettingValuesDGP);
    globalSealEngine->setEdcSchedule(EDCDGP.getGasSchedule(pindex->nHeight + 1));
#endif
    uint64_t minGasPrice = 0;//EDCDGP.getMinGasPrice(pindex->nHeight + 1);
    uint64_t blockGasLimit = DEFAULT_BLOCK_GAS_LIMIT_DGP;//EDCDGP.getBlockGasLimit(pindex->nHeight + 1);

#if 0
    uint32_t sizeBlockDGP = 0;//EDCDGP.getBlockSize(pindex->nHeight + 1);
...
///////////////////////////////////////////////////////////////////////////////////////// EDC
        if (pindex->nHeight >= Params().FirstSCBlock()) {
            ...
            if(tx.HasCreateOrCall() && !hasOpSpend) {
                ...
                EdcTxConverter convert(tx, &view, &block.vtx);

                ExtractEdcTX resultConvertEdcTX;
                if(!convert.extractionEdcTransactions(resultConvertEdcTX)) {
                ...
                if(!CheckMinGasPrice(resultConvertEdcTX.second, minGasPrice))
                ...
                dev::u256 gasAllTxs = dev::u256(0);
                ByteCodeExec exec(block, resultConvertEdcTX.first, blockGasLimit);
                ...
                for(EdcTransaction& ltx : resultConvertEdcTX.first) {
                ...
                {
                    for(size_t k = 0; k < resultConvertEdcTX.first.size(); k ++){
                        ...
                        tri.push_back(TransactionReceiptInfo{block.GetHash(pindex->nHeight >= Params().SwitchPhi2Block()), uint32_t(pindex->nHeight), tx.GetHash(), uint32_t(i), resultConvertEdcTX.first[k].from(), resultConvertEdcTX.first[k].to(),
...
bool static ConnectTip(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const CBlock* pblock)
{
    ...
    LogPrint("bench", "  - Load block from disk: %.2fms [%.2fs]\n", (nTime2 - nTime1) * 0.001, nTimeReadFromDisk * 0.000001);
    {
        ...
        if (pindexNew->nHeight >= chainparams.FirstSCBlock()) {
            oldHashStateRoot = dev::h256(globalState->rootHash()); // EDC
            oldHashUTXORoot = dev::h256(globalState->rootHashUTXO()); // EDC
        }
        ...
        if (!rv) {
            ...
            if (pindexNew->nHeight >= chainparams.FirstSCBlock()) {
                globalState->setRoot(oldHashStateRoot); // EDC
                globalState->setRootUTXO(oldHashUTXORoot); // EDC
            }
...
bool CheckForMasternodePayment(const CTransaction& tx, const CBlockHeader& header) {
    ...
    // Divide to keep a check precision of 0.01 EDC
 ...
 * As an example, Edccore uses /Edccore:X.X.X/ version string. So, this function will
 * return 5 for /Edccore:5.0.0/ or 0 if it couldn't find ":" or "." delimeters, or if no digits are between
 * those delimeters, or if string verison is empty
 * @param cleanVersion Edccore wallet version as a string
 * @return Major Edccore major version or 0
 */
...
std::vector<ResultExecute> CallContract(const dev::Address& addrContract, std::vector<unsigned char> opcode, const dev::Address& sender, uint64_t gasLimit){
    ...
    EdcTransaction callTransaction(0, 1, dev::u256(gasLimit), addrContract, opcode, dev::u256(0));
    callTransaction.forceSender(senderAddress);
    callTransaction.setVersion(VersionVM::GetEVMDefault());


    ByteCodeExec exec(block, std::vector<EdcTransaction>(1, callTransaction), blockGasLimit);
...
void writeVMlog(const std::vector<ResultExecute>& res, const CTransaction& tx, const CBlock& block){
    boost::filesystem::path EDCDir = GetDataDir() / "vmExecLogs.json";
    std::stringstream ss;
    if(fIsVMlogFile){
        ss << ",";
    } else {
        std::ofstream file(EDCDir.string(), std::ios::out | std::ios::app);
        file << "{\"logs\":[]}";
        file.close();
    }
    ...
    std::ofstream file(EDCDir.string(), std::ios::in | std::ios::out);
...
bool EdcTxConverter::extractionEdcTransactions(ExtractEdcTX& EDCtx){
    std::vector<EdcTransaction> resultTX;
    ...
    EDCtx = std::make_pair(resultTX, resultETP);
    return true;
}

bool EdcTxConverter::receiveStack(const CScript& scriptPubKey){
...
bool EdcTxConverter::parseEthTXParams(EthTransactionParams& params){
    try{
...
EdcTransaction EdcTxConverter::createEthTX(const EthTransactionParams& etp, uint32_t nOut){
    EdcTransaction txEth;
    if (etp.receiveAddress == dev::Address() && opcode != OP_CALL){
        txEth = EdcTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.code, dev::u256(0));
    }
    else{
        txEth = EdcTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.receiveAddress, etp.code, dev::u256(0));
    }
