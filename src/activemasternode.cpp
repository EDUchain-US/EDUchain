...
//
// Bootup the masternode, look for a 16120 Edc input and register on the network
//
...
vector<COutput> CActiveMasternode::SelectCoinsMasternode() {
    ...
    BOOST_FOREACH(const COutput& out, vCoins) {
        if (out.tx->vout[out.i].nValue == GetMNCollateral(chainActive.Tip()->nHeight) * COIN) {  //exactly DARKSEND_COLLATERAL EDC
...
vector <COutput> CActiveMasternode::SelectCoinsMasternodeForPubKey(std::string collateralAddress) {
    ...
    // Filter
    BOOST_FOREACH(const COutput& out, vCoins) {
        if (out.tx->vout[out.i].scriptPubKey == scriptPubKey && out.tx->vout[out.i].nValue == DARKSEND_COLLATERAL) { //exactly 161.200 EDC
            filteredCoins.push_back(out);
        }
    }
    return filteredCoins;
