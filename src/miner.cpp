...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx, bool fProofOfStake, int64_t* pTotalFees, int32_t txProofTime, int32_t nTimeLimit)
{
    ...
    globalSealEngine->setEdcSchedule(EDCDGP.getGasSchedule(nHeight));
    uint32_t blockSizeDGP = EDCDGP.getBlockSize(nHeight);
    minGasPrice = EDCDGP.getMinGasPrice(nHeight);
    ...
    hardBlockGasLimit = EDCDGP.getBlockGasLimit(nHeight);
...
bool BlockAssembler::AttemptToAddContractToBlock(CTxMemPool::txiter iter, uint64_t minGasPrice) {
    ...
    EdcTxConverter convert(iter->GetTx(), NULL, &pblock->vtx);

    ExtractEdcTX resultConverter;
    if(!convert.extractionEdcTransactions(resultConverter)){
    ...
    for(EdcTransaction EDCTransaction : EDCTransactions){
        txGas += EDCTransaction.gas();
        if(txGas > txGasLimit) {
            // Limit the tx gas limit by the soft limit if such a limit has been specified.
            return false;
        }

        if(bceResult.usedGas + EDCTransaction.gas() > softBlockGasLimit){
            //if this transaction's gasLimit could cause block gas limit to be exceeded, then don't add it
            return false;
        }
        if(EDCTransaction.gasPrice() < minGasPrice){
    ...
    ByteCodeExec exec(*pblock, EDCTransactions, hardBlockGasLimit);
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("EDCMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, chainparams, NULL, pblock)) {
        return error("EDCMiner : ProcessNewBlock, block not accepted");
    }
