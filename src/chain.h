...
public:
    ...
    unsigned int nBits;
    unsigned int nNonce;
    uint256 hashStateRoot; // EDC
    uint256 hashUTXORoot; // EDC
    ...
    void SetNull()
    {
        ...
        nBits = 0;
        nNonce = 0;
        hashStateRoot  = uint256(); // EDC
        hashUTXORoot   = uint256(); // EDC
    }
    ...
    CBlockIndex(const CBlock& block)
    {
        ...
        nBits = block.nBits;
        nNonce = block.nNonce;
        hashStateRoot  = block.hashStateRoot; // EDC
        hashUTXORoot   = block.hashUTXORoot; // EDC
    ...
    CBlockHeader GetBlockHeader() const
    {
        ...
        block.nBits = nBits;
        block.nNonce = nNonce;
        block.hashStateRoot  = hashStateRoot; // EDC
        block.hashUTXORoot   = hashUTXORoot; // EDC
        return block;
    }
...
public:
    ...
    inline void SerializationOp(Stream& s, Operation ser_action, int nType, int _nVersion)
    {
        ...
        if ((this->nVersion & (1 << 30)) != 0) {
            READWRITE(hashStateRoot);       // EDC
            READWRITE(hashUTXORoot);        // EDC
        }
    }
